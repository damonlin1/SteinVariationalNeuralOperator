...
for _ in range(1):
    ...
    optimizer = torch.optim.Adam([xout], lr=0.05, weight_decay=1e-5)
    scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=1000, gamma=0.5)
        
    for ep in range(epochs):
        # optimizer.zero_grad()
        
        out_masked = darcy_mask1(xout[::r, ::r])
        out_masked_threshold = darcy_mask2(xout[::r, ::r])
        testx_l2 = myloss(out_masked_threshold, x)
        ...
        testy_l2 = myloss(yout, y)
        loss_TV = total_variance(xout)
        
        
        pino_loss = testy_l2 + 0.05 * loss_TV
        pino_loss.backward()
        optimizer.step()
        scheduler.step()
        
        a_error.append(testx_l2.item())
        u_error.append(testy_l2.item())
            
        print(ep, testy_l2.item(), testx_l2.item())
        ...
        if ep == epochs - 1:
            ...
            plt.imshow(out_masked_threshold.reshape(ss, ss).detach().cpu().numpy())
